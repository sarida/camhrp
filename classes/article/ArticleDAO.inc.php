...
class ArticleDAO extends DAO {
	...
	function getLocaleFieldNames() {
		return array(
			'authorPhoneNumber', 'ethicsCommittee', 'scientificTitle', 'cleanScientificTitle', 'studentInitiatedResearch', 'studentInstitution', 'supervisorFullName', 'supervisorEmail', 'supervisorPhone', 'supervisorAffiliation', 'academicDegree', 'background', 'objectives', 'studyMatters', 'expectedOutcomes', 'coverPageAltText', 'showCoverPage', 'hideCoverPageToc', 'hideCoverPageAbstract', 'originalFileName', 'fileName', 'width', 'height', 'discipline', 'subjectClass', 'subject', 'coverageGeo', 'coverageChron', 'coverageSample', 'type', 'sponsor', 'keywords', 'startDate', 'endDate', 'fundsRequired', 'selectedCurrency', 'primarySponsor', 'secondarySponsors', 'multiCountryResearch', 'multiCountry', 'researchField', 'otherResearchField', 'withHumanSubjects', 'proposalType', 'otherProposalType', 'nationwide', 'proposalCountry', 'technicalUnit', 'dataCollection', 'submittedAsPi', 'conflictOfInterest', 'reviewedByOtherErc', 'otherErcDecision', 'rtoOffice', 'whoId', 'reasonsForExemption', 'withdrawReason', 'withdrawComments', 'approvalDate', 'industryGrant', 'nameOfIndustry', 'internationalGrant', 'internationalGrantName', 'mohGrant', 'governmentGrant', 'governmentGrantName', 'universityGrant', 'selfFunding', 'otherGrant', 'specifyOtherGrant', 'identityRevealed', 'unableToConsent', 'under18', 'dependentRelationship', 'ethnicMinority', 'impairment', 'pregnant', 'newTreatment', 'bioSamples', 'radiation', 'distress', 'inducements', 'sensitiveInfo', 'deception', 'reproTechnology', 'genetics', 'stemCell', 'biosafety', 'export', 'riskLevel', 'listRisks', 'howRisksMinimized', 'riskApplyTo', 'benefitsFromTheProject', 'multiInstitutions'
                        );
	}
	...
	function searchProposalsPublic($query, $dateFrom, $dateTo, $country, $status = null) {
		$searchSql = "select distinct 
				...
				engscientifictitle.setting_value as engscientifictitle,
				camscientifictitle.setting_value as camscientifictitle,
				...
				engkeywords.setting_value as engkeywords,
				camkeywords.setting_value as camkeywords
				  FROM articles a
				left join article_settings engscientifictitle on (engscientifictitle.article_id = a.article_id and engscientifictitle.setting_name = 'scientificTitle' and engscientifictitle.locale = 'en_US')
				left join article_settings camscientifictitle on (camscientifictitle.article_id = a.article_id and camscientifictitle.setting_name = 'scientificTitle' and engscientifictitle.locale = 'kh_CA')
				...
				left join article_settings engkeywords on (engkeywords.article_id = a.article_id and engkeywords.setting_name = 'keywords' and engkeywords.locale = 'en_US')
				left join article_settings camkeywords on (camkeywords.article_id = a.article_id and camkeywords.setting_name = 'keywords' and camkeywords.locale = 'kh_CA')
				inner join edit_decisions as ed on (ed.article_id = a.article_id)
				where (ed.decision = '1' || ed.decision = '6' || ed.decision = '9')";
		
		if (!empty($query)) {
			$searchSql .= " AND (engkeywords.setting_value LIKE '"
				."%".$query."%"
				."' or camkeywords.setting_value LIKE '"
				."%".$query."%"
				."' or engscientifictitle.setting_value LIKE '"
				."%".$query."%"
				."' or camscientifictitle.setting_value LIKE '"
				."%".$query."%"
				."')";
		}
	...
	function searchCustomizedProposalsPublic($query, $province, $statusFilter, $fromDate, $toDate, $investigatorName, $investigatorAffiliation, $investigatorEmail, $researchField, $proposalType, $duration, $area, $dataCollection, $status, $studentResearch, $primarySponsor, $fundsRequired, $dateSubmitted) {
		...
		$searchSqlMid = " FROM articles a 
			...
			left join article_settings camkeywords on (camkeywords.article_id = a.article_id and camkeywords.setting_name = 'keywords' and camkeywords.locale = 'kh_CA')  
			left join article_settings engscientifictitle on (engscientifictitle.article_id = a.article_id and engscientifictitle.setting_name = 'scientificTitle' and engscientifictitle.locale = 'en_US')
			left join article_settings camscientifictitle on (camscientifictitle.article_id = a.article_id and camscientifictitle.setting_name = 'scientificTitle' and camscientifictitle.locale = 'kh_CA')
			left join article_settings province on (province.article_id = a.article_id and province.setting_name = 'proposalCountry')
			left join article_settings startdate on (startdate.article_id = a.article_id and startdate.setting_name = 'startDate')";
		...
		if ($area == true){
			$searchSqlBeg .= ", province.setting_value as country, multicountry.setting_value as multicountryresearch, nationwide.setting_value as nationwide";
		...
		if (!empty($query)) {
			$searchSql .= " AND (engkeywords.setting_value LIKE '"."%".$query."%"."' or
				camkeywords.setting_value LIKE '"."%".$query."%"."' or
				engscientifictitle.setting_value LIKE '"."%".$query."%"."' or
				camscientifictitle.setting_value LIKE '"."%".$query."%"."')";
		}
		...
		if ($province != 'ALL') $searchSqlEnd .= " AND province.setting_value LIKE '"."%".$province."%"."'";
	...
	function _searchArticleFromRow(&$article, &$row) {
		...
		if (isset($row['camscientifictitle'])) $article->setScientificTitle($row['camscientifictitle'], 'kh_CA');
