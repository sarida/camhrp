...
class ReviewerSubmissionDAO extends DAO {
	...
	function getSubmissionsForFullReview($reviewerId){
		...
		$section == ' ';
		if ($user->isNechrMember() && !$user->isUhsMember()) $section = 'AND a.section_id = 1';
		else if (!$user->isNechrMember() && $user->isUhsMember()) $section = 'AND a.section_id = 2';
		$sql = 'SELECT a.*
			FROM articles a
...
/**
	...
	function getSubmissionsForERCReviewCount($reviewerId, $journalId) {
		...
		if ($user->isNechrMember() && !$user->isUhsMember()) $sql .= ' AND s.section_id = 1';
		else if (!$user->isNechrMember() && $user->isUhsMember()) $sql .= ' AND s.section_id = 2';
