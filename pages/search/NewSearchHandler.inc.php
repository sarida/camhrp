...
class NewSearchHandler extends Handler {
	...
	function advanced() {
        ...
        $proposalCountries =& $countryDAO->getProvincesOfCambodia();
	...
	function advancedResults() {
		...
		$templateMgr->assign('country', $countryDAO->getProvinceOfCambodia($country));
	...
	function generateCustomizedCSV($args) {
		...
		$results = $articleDao->searchCustomizedProposalsPublic($query, $province, $statusFilter, $fromDate, $toDate, $investigatorName, $investigatorAffiliation, $investigatorEmail, $researchField, $proposalType, $duration, $area, $dataCollection, $status, $studentResearch, $primarySponsor, $fundsRequired, $dateSubmitted);
	...
	function setupTemplate($subclass = false, $articleId = null) {
		...
		if ($articleId == null) {$templateMgr->assign('pageHierarchy',
			...
			$subclass ? array(array(Request::url(null, 'search'), 'navigation.search'), array(Request::url('camhrp', 'search','advancedResults'), 'search.searchResults'))
				: array()
			);
		}
