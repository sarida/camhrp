			...
			{foreach name="suppFiles" from=$suppFiles item=suppFile}
				{if $suppFile->getType() == 'Final Decision'}
					{if $suppFileDao->getSetting($suppFile->getSuppFileId(), 'investigator') == 'hide'}
						{translate key="article.suppFilesNotAuthorized"}
					{else}
                        <a href="{url op="downloadFile" path=$submission->getArticleId()|to_array:$suppFile->getFileId()}" class="file">{$suppFile->getFileName()|escape}</a>&nbsp;&nbsp;({$suppFile->getTypeTranslated()|escape})
					{/if}
				{else}
                    <a href="{url op="downloadFile" path=$submission->getArticleId()|to_array:$suppFile->getFileId()}" class="file">{$suppFile->getFileName()|escape}</a>&nbsp;&nbsp;({$suppFile->getTypeTranslated()|escape})
                    {if $canEditFiles}
