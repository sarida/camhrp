    ...
    if ($('#ethicsCommittee').val() != null) {ldelim}
        $.each(
            $('#ethicsCommittee').val(), function(key, value){ldelim}
                if(value == "NECHR" || value == "UHS"){ldelim}
...
function showOrHideButtons() {ldelim}
    ...
    if ($('#ercMemberStatus').val() != null) {ldelim}
        $.each(
            $('#ercMemberStatus').val(), function(key, value){ldelim}
                ...
                {rdelim}
                else if(value == "ExtReviewer") {ldelim}
    ...
    if (isUhsSelected) {ldelim}
    	...
    	{rdelim}
        	...
        	{rdelim} 
    	{rdelim}
    	else if(isExtReviewerSelected) {ldelim} 
        	$('#submit').show();
    	{rdelim}
    {rdelim}
    else if (isNechrSelected) {ldelim}
        if(isChairSelected) {ldelim}
    		$('#noMemberStatusSelected').hide();
    		if({$isNechrChair}=='1'){ldelim}
        		$('#submit').hide();
    	...
    	{rdelim}
    		...
    		$('#noMemberStatusSelected').hide();
    		if({$areNechrSecretary}=='1'){ldelim}
        		$('#submit').hide();
        		$('#nechrSecretaryErrorMessage').show();
        	{rdelim}
       		else if (checkCheckbox>{$freeNechrSecretaryPlaces}) {ldelim}
        		$('#submit').hide();
    	...
    	{rdelim}
    		...
    		$('#noMemberStatusSelected').hide();
    		if({$areNechrMembers}=='1'){ldelim}
        		$('#submit').hide();
        		$('#nechrMembersErrorMessage').show();
        	{rdelim}
        	else if (checkCheckbox>{$freeNechrMemberPlaces}) {ldelim}
        		$('#submit').hide();
...
{foreach from=$nechrChair item=nechrChair}
{$nechrChair->getFullName()|escape}<br />
{/foreach}
<font color=#FF0000>
Please unenroll him/her before enrolling someone else.
</font></p>
...
{foreach from=$nechrViceChair item=nechrViceChair}
{$nechrViceChair->getFullName()|escape}<br />
{/foreach}
<font color=#FF0000>
Please unenroll him/her before enrolling someone else.
</font></p>
...
{foreach from=$nechrSecretary item=nechrSecretary}
{$nechrSecretary->getFullName()|escape}<br />
{/foreach}
<font color=#FF0000>
Please unenroll at least one before enrolling someone else.
</font></p>
...
{foreach from=$nechrMembers item=nechrMembers}
{$nechrMembers->getFullName()|escape}<br />
{/foreach}
<font color=#FF0000>
Please unenroll at least one before enrolling someone else.
</font></p>
...
Only {$freeNechrSecretaryPlaces} place(s) available.
</font></p>
...
Only {$freeNechrMemberPlaces} place(s) available.
</font></p>
...
NECHR-ERC Member: {$freeNechrMemberPlaces} place(s) available.<br />
</font></p>
...
NECHR-ERC Secretary: {$freeNechrSecretaryPlaces} place(s) available.<br />
</font></p>
<!-- End of management of the ERC Status -->
