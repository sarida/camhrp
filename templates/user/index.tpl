	...
	<table width="100%" class="info">
		...
		{if $isValid.Editor.$journalId}
			<tr>
				...
				<td>&#187; <a href="{url journal=$journalPath page="editor" op="submissionsReport"}">{translate key="user.role.coordinator"}</a></td>
				<td></td>
				<td align="center"><a href="{url journal=$journalPath page="editor" op="submissionsReport"}">Report Generator</a></td>
		...
		{if $isValid.Reviewer.$journalId}
			...
			<tr>
				...
				{else}
				<td colspan="2">&#187; <a href="{url journal=$journalPath page="reviewer" path="fullReview"}">{$NECHR}</a></td>
				<td align="left">
	...
	{if $currentJournal}
		...
		<ul class="plain">
			...
			<li>
				&#187;
				{if $allowRegReviewer}
					{url|assign:"userHomeUrl" page="user" op="camhrp"}
